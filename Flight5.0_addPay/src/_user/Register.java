...
public class Register extends ManagerInfo{
...
//	private final static String DB_DRIVER = "com.mysql.jdbc.Driver";//jdbc
	...
	public static void register(){
		...
		if(!isLinked){
			try{
				DriverManager.registerDriver(new com.mysql.jdbc.Driver());//加载驱动
				conn = DriverManager.getConnection(DB_URL_INIT,DB_USER,DB_PW);//建立连接
				stmt = conn.createStatement();
				stmt.executeUpdate("create database flightsystem");
				stmt.close();
				conn.close();
			}catch(Exception e){
		...
		if(flag == 0){
			...
			String createTableNamePW = "create table "+USER_TABLE+"(id varchar(3),name varchar(10),password varchar(10),money varchar(10))"
			...
			try{
				...
				/*step  3   插入数据****************/
				String line= (userNumInDB+1)+","+"'"+name+"'"+","+"'"+pw+"'"+",'15'";
				String insertTableNamePW = "insert into "+USER_TABLE+"(id,name,password,money)"
                        + "values("+line+")";
				conn = DriverManager.getConnection(DB_URL,DB_USER,DB_PW);//建立连接
				stmt = conn.createStatement();
