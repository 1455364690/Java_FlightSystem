...
public class Book_tickets extends ManagerInfo{
	...
	public static int book(String num,String amount,String name){//航班号，购票数，用户名
		...
		strID = intID +"";
		String strMoney = GetMoney.getMoney(strID);
		//用户的钱
		...
		//
		int afterMoney = 0;
		int isBook = 2;//0状态表示购票成功，1状态表示航班号输入有误，2状态表示票数有误,3表示钱不够
		...
		try{
			...
			while(res.next()){
				if(res.getString("id").equals(num)){
					neededMoney = Integer.parseInt(res.getString("price"))*buyTic;
					maxTic = Integer.parseInt(res.getString("totaltickets"));
					soldTic = Integer.parseInt(res.getString("soldtickets"));
					//如果需要的钱大于用户要的钱则将状态置3
					if(neededMoney>intMoney){
						isBook = 3;
					}
					else if(soldTic+buyTic<=maxTic){//如果已售的票加上本次购买的票不大于最高票数，则在已售票加上本次购票数
						isBook = 0;
						soldTic +=buyTic;
						afterMoney = neededMoney;
					}else{
						isBook = 2;//否则信号置1
					} 
				}
		...
		if(isBook == 0){//如果购票成功
			/*阶段1，更改用户的钱*/
			ChangeMoney.delMoney(afterMoney,strID);
			try{
